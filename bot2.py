...
TEXTS = {
    'fa': {
        ...
        'daily_success': "🎉 **پاداش روزانه!** ۲ امتیاز به حساب شما اضافه شد.",
        'daily_already': "⚠️ شما پاداش امروز را قبلاً دریافت کرده‌اید! فردا دوباره تلاش کنید.",
        'buy_text': (
            "💳 ━━━━━━━━━━━━━━━━━━ 💳\n"
            ...
            "💎 **۳۰ امتیاز** 👈 **۸۰ افغانی**\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📩 **جهت سفارش و خرید امتیاز، به پشتیبانی پیام دهید.**"
        ),
        'info_text': "👤 **اطلاعات حساب:**\nآیدی: `{id}`\nامتیاز: `{points}`\n\n🔗 **لینک دعوت شما:**\n`https://t.me/{bot}?start=ref_{id}`\n\n🎁 با دعوت هر دوست ۲ امتیاز بگیرید!",
        'send_file_req': "📂 **لطفاً اکنون فایل پایتون (.py) خود را ارسال کنید:**",
        'low_points': "❌ **امتیاز شما کافی نیست!** (حداقل ۱۰ امتیاز لازم است)",
        'select_lang': "لطفاً زبان خود را انتخاب کنید / مهرباني وکړئ خپله ژبه وټاکئ / Please select your language:"
    },
    'ps': {
        'welcome': "✨ سلام {name} ګرانه! ربات ته ښه راغلاست.\n\n📋 **ستاسو د حساب مشخصات:**\n👤 **نوم:** {name}\n🆔 **یوزرنیم:** {username}\n🔢 **عددي آئي ډي:** `{id}`\n💎 **امتیازونه:** `{points}` امتیاز",
        ...
        'buy_text': (
            ...
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "💎 **۱۰ امتیازونه** 👈 **۵۰ افغانۍ**\n"
            "💎 **۲۰ امتیازونه** 👈 **۷۰ افغانۍ**\n"
            "💎 **۳۰ امتیازونه** 👈 **۸۰ افغانۍ**\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📩 **د اخیستلو لپاره د پشتیبانۍ برخې ته پیام واستوئ.**"
        ),
        'info_text': "👤 **ستاسو معلومات:**\nآئي ډي: `{id}`\nامتیازونه: `{points}`\n\n🔗 **ستاسو د بلنې لینک:**\n`https://t.me/{bot}?start=ref_{id}`",
        'send_file_req': "📂 **مهرباني وکړئ خپل د پایتون (.py) فایل واستوئ:**",
        'low_points': "❌ **ستاسو امتیازونه کافي ندي!** (لږترلږه ۱۰ امتیازونه پکار دي)",
    ...
    'en': {
        ...
        'buy_text': (
            ...
            "💎 **30 Points** 👈 **80 AFN**\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📩 **To purchase points, please contact support.**"
        ),
        'info_text': "👤 **Your Info:**\nID: `{id}`\nPoints: `{points}`\n\n🔗 **Referral Link:**\n`https://t.me/{bot}?start=ref_{id}`",
...
def add_points_cmd(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ **تعداد `{amount}` امتیاز به کاربر `{target_id}` اضافه شد.**\n💎 **موجودی کل جدید:** `{new_total}` امتیاز", parse_mode="Markdown")
        try:
            bot.send_message(target_id, f"🎉 **تبریک!**\nتعداد `{amount}` امتیاز از طرف مدیریت به حساب شما اضافه شد.\n💎 **موجودی کل جدید شما:** `{new_total}` امتیاز", parse_mode="Markdown")
        except Exception:
            pass
    except Exception:
        bot.reply_to(message, "⚠️ **طرز استفاده درست:**\n`/add 8173349543 10`", parse_mode="Markdown")
...
def admin_panel(message):
    ...
    btn_text_bc = types.InlineKeyboardButton("📝 ارسال پیام دلخواه به همه", callback_data="bc_text", style="primary")
    btn_fwd_bc = types.InlineKeyboardButton("🔄 فوروارد پیام به همه", callback_data="bc_fwd", style="primary")
...
def handle_admin_broadcast_choice(call):
    ...
    if call.data == "bc_text":
        ...
        bot.send_message(call.message.chat.id, "✍️ **لطفاً متن پیام دلخواه خود را ارسال کنید:**", parse_mode="Markdown")
    else:
        waiting_for_broadcast[call.message.chat.id] = "fwd"
        bot.send_message(call.message.chat.id, "🔄 **لطفاً پیامی را که می‌خواهید فوروارد شود ارسال کنید:**", parse_mode="Markdown")

# پردازش ارسال همگانی
@bot.message_handler(func=lambda m: is_admin(m.chat.id) and m.chat.id in waiting_for_broadcast)
def process_admin_broadcast(message):
    ...
    bot.send_message(message.chat.id, f"✅ **عملیات پایان یافت!**\n\n🟢 **موفق:** `{success}`\n🔴 **ناموفق:** `{failed}`", parse_mode="Markdown")
...
def start_cmd(message):
    ...
    if is_new and len(text_args) > 1 and text_args[1].startswith("ref_"):
        try:
            ...
            if ref_id != chat_id and chat_id not in invited_users:
                ...
                bot.send_message(ref_id, f"🎉 یک کاربر با لینک شما عضو شد! ۲ امتیاز دریافت کردید.\n💎 **موجودی:** `{user_points[ref_id]}`", parse_mode="Markdown")
        except Exception:
            pass
    ...
    bot.send_message(chat_id, welcome_msg, reply_markup=main_keyboard(chat_id), parse_mode="Markdown")

# ----------------- مدیریت کلیک روی دکمه‌های اصلی -----------------
...
def handle_text_buttons(message):
    ...
    # تغییر زبان
    if text in [TEXTS['fa']['btn_lang'], TEXTS['ps']['btn_lang'], TEXTS['en']['btn_lang']]:
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("🇦🇫 دری / فارسی", callback_data="setlang_fa", style="primary"),
            types.InlineKeyboardButton("🇦🇫 پښتو", callback_data="setlang_ps", style="primary"),
            types.InlineKeyboardButton("🇬🇧 English", callback_data="setlang_en", style="primary")
        )
        bot.send_message(chat_id, t['select_lang'], reply_markup=markup)
        return

    # سکه روزانه (۲ امتیاز)
    if text in [TEXTS['fa']['btn_daily'], TEXTS['ps']['btn_daily'], TEXTS['en']['btn_daily']]:
    ...
    # خرید امتیاز
    if text in [TEXTS['fa']['btn_buy'], TEXTS['ps']['btn_buy'], TEXTS['en']['btn_buy']]:
        bot.send_message(chat_id, t['buy_text'], parse_mode="Markdown")
        return

    # اطلاعات من / لینک دعوت
    if text in [TEXTS['fa']['btn_info'], TEXTS['ps']['btn_info'], TEXTS['en']['btn_info']]:
        info = t['info_text'].format(id=chat_id, points=get_points(chat_id), bot=BOT_USERNAME)
        bot.send_message(chat_id, info, parse_mode="Markdown")
        return

    # پشتیبانی
    if text in [TEXTS['fa']['btn_support'], TEXTS['ps']['btn_support'], TEXTS['en']['btn_support']]:
        waiting_for_support.add(chat_id)
        bot.send_message(chat_id, "📥 لطفاً پیام خود را بنویسید تا به مدیر ارسال شود:")
        return

    # تغییر رنگ کیبورد
    if text in [TEXTS['fa']['btn_color'], TEXTS['ps']['btn_color'], TEXTS['en']['btn_color']]:
    ...
    if chat_id in waiting_for_support:
        ...
        user = message.from_user
        fwd_text = f"📩 **پیام پشتیبانی از طرف:** {user.first_name}\n🆔 **آیدی:** `{chat_id}`\n\n💬 **متن:**\n{message.text}"
...
def callback_set_language(call):
    ...
    bot.send_message(chat_id, "✅ Language updated!", reply_markup=main_keyboard(chat_id))
...
def handle_file(message):
    ...
    matches = re.findall(r"types\.InlineKeyboardButton\s*\((.*?)\)", file_content, flags=re.DOTALL)
    ...
    bot.send_message(chat_id, f"✅ فایل آنالیز شد. ۱۰ امتیاز کسر گردید.\nتعداد دکمه‌ها: `{len(matches)}`", parse_mode="Markdown")
...
def ask_button_color(chat_id):
    ...
    btn_label = btn_label_match.group(1) if btn_label_match else f"دکمه {index + 1}"

    markup = types.InlineKeyboardMarkup(row_width=3)
    markup.add(
        types.InlineKeyboardButton("🔴 قرمز", callback_data="color_danger", style="danger"),
        types.InlineKeyboardButton("🟢 سبز", callback_data="color_success", style="success"),
        types.InlineKeyboardButton("🔵 آبی", callback_data="color_primary", style="primary")
    )
    bot.send_message(chat_id, f"🎨 **رنگ دکمه ({index + 1} از {total}):** `{btn_label}`", reply_markup=markup, parse_mode="Markdown")
...
def build_and_send_final_file(chat_id):
    ...
    def replace_button(match):
        ...
        btn_inner = re.sub(r',\s*style\s*=\s*["\'].*?["\']', '', btn_inner, flags=re.DOTALL)
        
        assigned_style = styles[btn_counter]
        btn_counter += 1
        return f"types.InlineKeyboardButton({btn_inner.strip()}, {assigned_style})"

    # جایگزینی دکمه‌ها در تمام خطوط
    new_content = re.sub(r"types\.InlineKeyboardButton\s*\((.*?)\)", replace_button, content, flags=re.DOTALL)
